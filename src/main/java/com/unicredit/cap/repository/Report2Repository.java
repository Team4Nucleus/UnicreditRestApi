...
public interface Report2Repository extends JpaRepository<Report2, String>{
		...
		@Query(value = 
				" WITH DATA as (																																											" +
						...
						" SUM (CASE WHEN  p.DECISION = 'POZITIVNO' or p.DECISION = 'USLOVNO POZITIVNO' then 1 else 0 end) as BrojOdobrenihPlasmana,                                                                 " +
						" SUM (CASE WHEN p.DECISION = 'POZITIVNO' or p.DECISION = 'USLOVNO POZITIVNO' then AMOUNT_IN_BAM_APPROVED else 0 end) as IznosOdobrenihPlasmana,                                            " +
						" SUM (CASE WHEN p.DECISION = 'NEGATIVNO' then 1 else 0 end) as BrojOdbijenihPlasmana,                                                                                                      " +
						" SUM (CASE WHEN p.DECISION = 'POZITIVNO' then AMOUNT_IN_BAM else 0 end) as IznosOdbijenihPlasmana,                                                                                         " +
						" SUM (CASE WHEN p.DECISION IS NOT NULL then  CALCULATE_TIME(p.CREATING_DATE, p.DECISION_DATE ) else 0 end ) as TimeToDecision,                                                             " +
						" SUM (CASE when p.STATUS IN (32,33)  AND ( p.DECISION = 'POZITIVNO' or p.DECISION = 'USLOVNO POZITIVNO' ) then 1 else 0 end ) as Isplaceno,                                                " +
						" SUM (CASE when p.STATUS IN (32,33)  AND ( p.DECISION = 'POZITIVNO' or p.DECISION = 'USLOVNO POZITIVNO' ) then CALCULATE_TIME(p.CREATING_DATE, p.CLOSING_DATE ) else 0 end) as TimeToCash, " +
